...
The SDK is now generated by Fern and developed in the `flora-frontend`
monorepo (`sdk/typescript`). This release changes the client shape; upgrading
from 0.11 and earlier:

- Import the named `FloraClient` (`import { FloraClient } from "@flora-ai/flora"`);
  there is no default export. `apiKey` / `FLORA_API_KEY` are unchanged.
- `baseURL` → `baseUrl`; `defaultHeaders` → `headers`; `timeout` (ms) →
  `timeoutInSeconds`.
- Path parameters are passed in the request object (`client.assets.retrieve({ assetId })`)
  instead of positionally.
- `APIError` and its subclasses → `FloraError` and per-status classes under the
  `Flora` namespace (`Flora.NotFoundError`, …).
- Paginated lists return a `Page` (async iterable with `.data`, `.hasNextPage()`,
  `.getNextPage()`).
- `assets.upload` accepts a URL, file path, `File`/`Blob`, or bytes; stream inputs
  are no longer accepted.
- Renamed: `workspaces.projects.canvas.retrieveDefinition/replaceDefinition/applyChangeset`
  and `workspaces.projects.graph/runNodes` → `projects.canvas.getDefinition/
replaceDefinition/applyChangeset/getGraph/runNodes`; `workspaces.folders.createProject`
  → `workspaces.projects.createInFolder`; `workspaces.library.folders.create/delete/addItem`
  → `workspaces.library.createFolder/deleteFolder/addFolderItem`;
  `projects.assets.attachAsset` → `projects.assets.attach`. Other resource groups
  and method names are unchanged.
- New: `techniques.create` (`POST /techniques`).
- Removed: `runs.startTechnique` (`POST /runs/technique`); use `techniques.runs.create`.
- The `flora-mcp` MCP server is no longer built from this package.
