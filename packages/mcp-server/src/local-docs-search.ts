...
const EMBEDDED_METHODS: MethodEntry[] = [
  {
    ...
    response:
      "{ inputs: { id: string; name: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; description?: string; specifiedAspectRatio?: string; specifiedDuration?: number; }[]; name: string; outputs: { id: string; name: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; description?: string; specifiedAspectRatio?: string; specifiedDuration?: number; }[]; run_cost: number; technique_id: string; description?: string; }",
    markdown:
      "## retrieve\n\n`client.techniques.retrieve(techniqueId: string): { inputs: object[]; name: string; outputs: object[]; run_cost: number; technique_id: string; description?: string; }`\n\n**get** `/techniques/{techniqueId}`\n\nReturns the public definition for one technique, including its input and output schema used to start runs.\n\n### Parameters\n\n- `techniqueId: string`\n  Technique identifier or slug\n\n### Returns\n\n- `{ inputs: { id: string; name: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; description?: string; specifiedAspectRatio?: string; specifiedDuration?: number; }[]; name: string; outputs: { id: string; name: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; description?: string; specifiedAspectRatio?: string; specifiedDuration?: number; }[]; run_cost: number; technique_id: string; description?: string; }`\n\n  - `inputs: { id: string; name: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; description?: string; specifiedAspectRatio?: string; specifiedDuration?: number; }[]`\n  - `name: string`\n  - `outputs: { id: string; name: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; description?: string; specifiedAspectRatio?: string; specifiedDuration?: number; }[]`\n  - `run_cost: number`\n  - `technique_id: string`\n  - `description?: string`\n\n### Example\n\n```typescript\nimport Flora from '@flora-ai/sdk';\n\nconst client = new Flora();\n\nconst technique = await client.techniques.retrieve('tech_def_abc123');\n\nconsole.log(technique);\n```",
    perLanguage: {
      typescript: {
      ...
      http: {
        example:
          'curl https://api.example.com/techniques/$TECHNIQUE_ID \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example: 'curl https://api.example.com/techniques \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    params: [
      'techniqueId: string;',
      "inputs: { id: string; type: 'imageUrl' | 'videoUrl' | 'text'; value: string; }[];",
      "mode: 'async' | 'stream';",
      'callback_url?: string;',
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.example.com/techniques/$TECHNIQUE_ID/runs \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $FLORA_API_KEY" \\\n    -d \'{\n          "inputs": [\n            {\n              "id": "id",\n              "type": "imageUrl",\n              "value": "value"\n            }\n          ],\n          "mode": "async"\n        }\'',
      },
    },
  },
  {
    ...
    response:
      "{ createdAt: number; progress: number; runId: string; status: 'pending' | 'running' | 'completed' | 'failed'; chargedCost?: number; completedAt?: number; errorCode?: string; errorMessage?: string; outputs?: { outputId: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; url: string; }[]; pollUrl?: string; startedAt?: number; }",
    markdown:
      "## retrieve\n\n`client.techniques.runs.retrieve(techniqueId: string, runId: string): { createdAt: number; progress: number; runId: string; status: 'pending' | 'running' | 'completed' | 'failed'; chargedCost?: number; completedAt?: number; errorCode?: string; errorMessage?: string; outputs?: object[]; pollUrl?: string; startedAt?: number; }`\n\n**get** `/techniques/{techniqueId}/runs/{runId}`\n\nReturns status, progress, outputs, and error details for a technique run when it is accessible to the authenticated public API key.\n\n### Parameters\n\n- `techniqueId: string`\n  Technique identifier or slug\n\n- `runId: string`\n  Run identifier\n\n### Returns\n\n- `{ createdAt: number; progress: number; runId: string; status: 'pending' | 'running' | 'completed' | 'failed'; chargedCost?: number; completedAt?: number; errorCode?: string; errorMessage?: string; outputs?: { outputId: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; url: string; }[]; pollUrl?: string; startedAt?: number; }`\n\n  - `createdAt: number`\n  - `progress: number`\n  - `runId: string`\n  - `status: 'pending' | 'running' | 'completed' | 'failed'`\n  - `chargedCost?: number`\n  - `completedAt?: number`\n  - `errorCode?: string`\n  - `errorMessage?: string`\n  - `outputs?: { outputId: string; type: 'imageUrl' | 'videoUrl' | 'audioUrl' | 'text' | 'documentUrl'; url: string; }[]`\n  - `pollUrl?: string`\n  - `startedAt?: number`\n\n### Example\n\n```typescript\nimport Flora from '@flora-ai/sdk';\n\nconst client = new Flora();\n\nconst run = await client.techniques.runs.retrieve('run_abc123', { techniqueId: 'tech_def_abc123' });\n\nconsole.log(run);\n```",
    perLanguage: {
      typescript: {
      ...
      http: {
        example:
          'curl https://api.example.com/techniques/$TECHNIQUE_ID/runs/$RUN_ID \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    response:
      "{ asset_id: string; status: 'pending_upload' | 'ready' | 'failed'; url: string; visibility: 'workspace'; workspace_id: string; expires_at?: string; failure_message?: string; upload?: { contentType: 'multipart/form-data'; fileField: 'file'; formFields: object; method: 'POST'; url: string; }; upload_url?: string; }",
    markdown:
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.example.com/assets/$ASSET_ID/complete \\\n    -X POST \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.example.com/assets/$ASSET_ID/retry \\\n    -X POST \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example: 'curl https://api.example.com/assets \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.example.com/assets/$ASSET_ID \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example: 'curl https://api.example.com/workspaces \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example: 'curl https://api.example.com/projects \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
  ...
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.example.com/projects/$PROJECT_ID \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.example.com/projects/$PROJECT_ID/nodes \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    perLanguage: {
      ...
      http: {
        example:
          'curl https://api.example.com/projects/$PROJECT_ID/assets/$ASSET_ID/attach \\\n    -X POST \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ models: { capabilities: string[]; estimated_credits: number; estimated_seconds: number; model_id: string; name: string; provider: string; type: 'image' | 'video' | 'audio' | 'text'; beta?: boolean; }[]; }",
    markdown:
    ...
    perLanguage: {
      ...
      http: {
        example: 'curl https://api.example.com/models \\\n    -H "Authorization: Bearer $FLORA_API_KEY"',
      },
    },
  },
  {
    ...
    response:
      "{ charged_cost: number; estimated_seconds: number; run_id: string; type: 'generation' | 'technique'; workflow_run_id: string; model?: { model_id: string; }; poll_url?: string; project_id?: string; technique?: { name: string; technique_id: string; }; }",
    markdown:
      "## start_generation\n\n`client.runs.startGeneration(project_id: string, prompt: string, type: 'image' | 'video' | 'audio' | 'text', workspace_id: string, model?: string, params?: object): { charged_cost: number; estimated_seconds: number; run_id: string; type: 'generation' | 'technique'; workflow_run_id: string; model?: object; poll_url?: string; project_id?: string; technique?: object; }`\n\n**post** `/runs/generation`\n\nStarts a model generation run in a project canvas using a prompt, workspace, project, optional model, and optional model parameters. Mutating public API requests support an optional Idempotency-Key header for client retries; duplicate keys within two hours return idempotency_duplicate.\n\n### Parameters\n\n- `project_id: string`\n  Project identifier\n\n- `prompt: string`\n  Generation prompt\n\n- `type: 'image' | 'video' | 'audio' | 'text'`\n  Generation type\n\n- `workspace_id: string`\n  Workspace identifier\n\n- `model?: string`\n  Model endpoint ID\n\n- `params?: object`\n  Model parameters\n\n### Returns\n\n- `{ charged_cost: number; estimated_seconds: number; run_id: string; type: 'generation' | 'technique'; workflow_run_id: string; model?: { model_id: string; }; poll_url?: string; project_id?: string; technique?: { name: string; technique_id: string; }; }`\n\n  - `charged_cost: number`\n  - `estimated_seconds: number`\n  - `run_id: string`\n  - `type: 'generation' | 'technique'`\n  - `workflow_run_id: string`\n  - `model?: { model_id: string; }`\n  - `poll_url?: string`\n  - `project_id?: string`\n  - `technique?: { name: string; technique_id: string; }`\n\n### Example\n\n```typescript\nimport Flora from '@flora-ai/sdk';\n\nconst client = new Flora();\n\nconst response = await client.runs.startGeneration({\n  project_id: 'prj_abc123',\n  prompt: 'A cinematic product photo of a ceramic mug on a sunlit table',\n  type: 'image',\n  workspace_id: 'ws_abc123',\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
      ...
      http: {
        example:
          'curl https://api.example.com/runs/generation \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $FLORA_API_KEY" \\\n    -d \'{\n          "project_id": "prj_abc123",\n          "prompt": "A cinematic product photo of a ceramic mug on a sunlit table",\n          "type": "image",\n          "workspace_id": "ws_abc123",\n          "model": "t2i-flux-2-pro"\n        }\'',
      },
    },
  },
  {
    ...
    response:
      "{ charged_cost: number; estimated_seconds: number; run_id: string; type: 'generation' | 'technique'; workflow_run_id: string; model?: { model_id: string; }; poll_url?: string; project_id?: string; technique?: { name: string; technique_id: string; }; }",
    markdown:
      "## start_technique\n\n`client.runs.startTechnique(inputs: object, technique_id: string, workspace_id: string): { charged_cost: number; estimated_seconds: number; run_id: string; type: 'generation' | 'technique'; workflow_run_id: string; model?: object; poll_url?: string; project_id?: string; technique?: object; }`\n\n**post** `/runs/technique`\n\nStarts a technique run through the normalized top-level run resource. Mutating public API requests support an optional Idempotency-Key header for client retries; duplicate keys within two hours return idempotency_duplicate.\n\n### Parameters\n\n- `inputs: object`\n  Technique inputs\n\n- `technique_id: string`\n  Technique identifier\n\n- `workspace_id: string`\n  Workspace identifier\n\n### Returns\n\n- `{ charged_cost: number; estimated_seconds: number; run_id: string; type: 'generation' | 'technique'; workflow_run_id: string; model?: { model_id: string; }; poll_url?: string; project_id?: string; technique?: { name: string; technique_id: string; }; }`\n\n  - `charged_cost: number`\n  - `estimated_seconds: number`\n  - `run_id: string`\n  - `type: 'generation' | 'technique'`\n  - `workflow_run_id: string`\n  - `model?: { model_id: string; }`\n  - `poll_url?: string`\n  - `project_id?: string`\n  - `technique?: { name: string; technique_id: string; }`\n\n### Example\n\n```typescript\nimport Flora from '@flora-ai/sdk';\n\nconst client = new Flora();\n\nconst response = await client.runs.startTechnique({\n  inputs: { foo: 'bar' },\n  technique_id: 'tech_abcd1234',\n  workspace_id: 'ws_abc123',\n});\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
      ...
      http: {
        example:
          'curl https://api.example.com/runs/technique \\\n    -H \'Content-Type: application/json\' \\\n    -H "Authorization: Bearer $FLORA_API_KEY" \\\n    -d \'{\n          "inputs": {\n            "foo": "bar"\n          },\n          "technique_id": "tech_abcd1234",\n          "workspace_id": "ws_abc123"\n        }\'',
      },
    },
  },
  {
