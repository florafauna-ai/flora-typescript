...
import { Flora, FloraClient } from "../index.js"
...
describe("assets.upload helper (3-path)", () => {
  test("small local file → direct multipart bytes in one call (no reserve/complete)", async () => {
    const { client, calls } = makeClient(async (url, method, init) => {
  ...
  test("large local file (>4MB) → signed-url reserve → upload → complete → poll", async () => {
    ...
    expect(asset.status).toBe("ready")
    expect(
      calls.some((c) => c.url === "https://storage.flora.test/signed-post" && c.method === "POST"),
    ).toBe(true)
    expect(calls.some((c) => c.url.endsWith("/assets/asset_big/complete"))).toBe(true)
    expect(polls).toBe(2)
  })

  test("serialized upload_url fallback → decoded into a multipart POST with the policy fields", async () => {
    let providerInit: RequestInit | undefined
    const { client } = makeClient(async (url, method, init) => {
      if (url.endsWith("/assets") && method === "POST") {
        return jsonResponse({
          asset_id: "asset_legacy",
          status: "pending_upload",
          uploaded_via: "signed_url",
          url: "https://cdn.flora.test/asset_legacy",
          visibility: "workspace",
          workspace_id: "ws_abc123",
          upload_url: JSON.stringify({
            url: "https://storage.flora.test/legacy-post",
            fields: { key: "uploads/asset_legacy", policy: "p" },
          }),
        })
      }
      if (url === "https://storage.flora.test/legacy-post") {
        providerInit = init
        return new Response(null, { status: 204 })
      }
      if (url.endsWith("/assets/asset_legacy/complete") && method === "POST") {
        return jsonResponse({ asset_id: "asset_legacy", status: "pending_upload" })
      }
      if (url.endsWith("/assets/asset_legacy") && method === "GET") {
        return jsonResponse(readyAsset("asset_legacy", { uploaded_via: "signed_url" }))
      }
      throw new Error(`unexpected request: ${method} ${url}`)
    })
    ...
    expect(form.get("key")).toBe("uploads/asset_legacy")
    expect(form.get("policy")).toBe("p")
    expect(form.get("file")).toBeInstanceOf(File)
    // The caller's cancellation reaches the provider transfer, not just the API calls.
    expect(providerInit?.signal).toBeInstanceOf(AbortSignal)
  })

  test("direct upload maps API status codes to the generated error classes", async () => {
    const { client } = makeClient(async (url, method) => {
      if (url.endsWith("/assets") && method === "POST") {
        return jsonResponse({ error: { code: "insufficient_credits", message: "no credits" } }, 402)
      }
      throw new Error(`unexpected request: ${method} ${url}`)
    })
    await expect(
      client.assets.upload(Buffer.from("png"), { workspace_id: "ws_abc123", file_name: "a.png" }),
    ).rejects.toBeInstanceOf(Flora.PaymentRequiredError)
  })

  test("plain http:// source is rejected locally", async () => {
    const { client, calls } = makeClient(async (url, method) => {
      throw new Error(`unexpected request: ${method} ${url}`)
    })
    await expect(
      client.assets.upload("http://example.com/image.png", { workspace_id: "ws_abc123" }),
