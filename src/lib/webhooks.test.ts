...
import { FloraClient, type WebhookEvent, WebhookVerificationError } from "../index.js"
...
const client = new FloraClient({ apiKey: "ak_test" })
...
describe("client.webhooks.unwrap", () => {
  ...
  test("rejects when the signature header is missing", async () => {
    ...
    await expect(client.webhooks.unwrap(body, {}, SECRET, { now })).rejects.toThrow(
      /Flora-Signature/,
    )
  })
  ...
  test("rejects when no secret is provided", async () => {
    ...
    await expect(
      client.webhooks.unwrap(body, { "Flora-Signature": signature }, "", { now }),
  ...
  test("verifies across secret rotation (multiple v1 signatures)", async () => {
    ...
    const event = await client.webhooks.unwrap(body, { "Flora-Signature": combined }, SECRET, {
      now,
    })
    expect(event.id).toBe("whd_abc123")
  })
