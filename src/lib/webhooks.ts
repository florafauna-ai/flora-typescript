...
//
// FLORA signs every delivery with HMAC-SHA256 over `${t}.${rawBody}` using the
// per-workspace signing secret (`whsec_…`) and sends it in a Stripe-style
// `Flora-Signature` header: `t=<unix-seconds>,v1=<hex>`. Verification checks
// the signature (constant-time) and the timestamp (replay window) before
// returning the parsed, typed event.
//
// Pass the EXACT raw request body — re-serializing parsed JSON breaks the match.
...
/**
 * Anything you can hand to {@link Webhooks.unwrap} as the headers argument: a
 * `Headers` instance, a plain headers record (e.g. Node's `req.headers`), or the
 * raw `Flora-Signature` header value.
 */
...
export interface WebhookUnwrapOptions {
   ...
   * Set to `0` to disable the timestamp/replay check entirely.
   */
...
/**
 * Verifies and parses FLORA webhook callbacks. Accessed as `client.webhooks`.
 * Verification is stateless and makes no network calls.
 */
  ...
  async verify(
    ...
    if (!secret) {
      throw new WebhookVerificationError(
        "A webhook signing secret is required to verify signatures.",
      )
    }
    ...
    if (timestamp === null) {
      throw new WebhookVerificationError(
        `No timestamp (t=) found in the ${SIGNATURE_HEADER} header.`,
      )
    }
...
function getSignatureHeader(headers: WebhookHeaders): string | null {
  ...
  if (headers instanceof Headers) return headers.get(SIGNATURE_HEADER)
...
async function getSubtleCrypto(): Promise<SubtleCrypto> {
  ...
  try {
    ...
    if (subtle) return subtle as SubtleCrypto
  } catch {
    // Fall through to the error below.
  }
  throw new FloraError({
    message: "The Web Crypto API is not available; cannot verify webhook signatures.",
  })
}
